...
void main(){
  ...
  // CHANGED: To make it look like hanging from top, we shift the fbm or base
  // The user wanted "handing from top and animation in a cave stones attached to ceiling"
  // So we can increase density at the top. 
  // 'uv.y' goes from positive at top to negative at bottom? Wait, gl_FragCoord.xy goes 0,0 at bottom left.
  // So normalized y is gl_FragCoord.y / uResolution.y
  
  float base = fbm2(uv, uTime * 0.05);
  base = base * 0.5 - 0.65;
  
  // Add ceiling density
  float normY = gl_FragCoord.y / uResolution.y; 
  // Increase feed linearly towards top (normY = 1.0)
  float ceilingBoost = smoothstep(0.3, 1.0, normY) * 0.8;

  float feed = base + (uDensity - 0.5) * 0.3 + ceilingBoost;
